...
/*   Updated: 2017/10/29 18:45:49 by sflinois         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	new_room(char *line, t_struct *s, int place)
{
	...
	i = 0;
	if (place == 1 && ft_strcmp(line, "##start") == 0)
		return (2);
	else if (place == 1 && ft_strcmp(line, "##end") == 0)
		return (3);
	if (*line == '#' && place == 1)
		return (1);
...
int	pars_args(int argc, char **argv, t_struct *s)
{
	...
	while (get_next_line(fd, &line) > 0 && line_type >= 0 && argc == 2)
	{
		...
		if (line_type > 0)
			s->anthill = ft_strjoin_free(s->anthill,
				ft_strjoin_free(line, ft_strdup("\n")));
	}
	close(fd);
